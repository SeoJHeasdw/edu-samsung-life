...
description: 산업 신호를 스크리닝하고 밸류체인과 비즈니스 모델 후보를 구조화한 뒤, 시장성·적시성·전략적합성을 근거 기반으로 채점하고 심층검토 보고서를 만드는 워크플로우. 신사업 발굴, 섹터 스크리닝, 밸류체인 매핑, BM 채점, 시장·기업·규제 조사, 제휴·인수 후보 탐색, 팩트체크 또는 신사업 보고서 작성을 요청할 때 사용한다. 공개·내부 데이터를 연결한 실제 조사와 번들 mock data를 이용한 안전한 데모를 모두 지원한다.
---
...
3. 요청이 `데모`, `mock`, `샘플`이면 번들 데이터를 사용한다. 그 외에는 실제 조사 모드로 취급한다.
4. 결과 디렉터리를 정한다. 사용자가 지정하지 않으면 임시 디렉터리를 사용한다.
5. 아래 명령의 `<skill-dir>`를 이 `SKILL.md`가 있는 절대 경로로 바꾼다.
