...
1. `references/methodology.md`를 읽어 점수 기준과 HITL를 적용한다.
...
- 참가자가 평가기준 하나와 HITL 하나를 바꾸고 전후 결과를 비교하게 한다.
...
- HITL는 생략하지 말고 `데모 기본값으로 통과`라고 기록한다.
...
- **HITL 1:** 조사 범위와 심층 분석할 섹터를 승인받는다. 데모에서는 기본값을 사용한다.
...
- **HITL 2:** 평가 기준, 가중치와 1차 후보를 승인받는다.
...
- **HITL 3:** 가정, 누락, 반대 근거와 중대한 리스크를 검토한다.
...
- **HITL 4:** 투자·제휴·보류의 최종 결정은 사용자에게 남긴다.
